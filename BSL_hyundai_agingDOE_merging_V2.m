...
%상위 폴더 경로 지정
rootfolder_path = 'G:\공유 드라이브\BSL_Data3\HNE_AgingDOE_Processed\HNE_FCC';
...
for i = 1:length(cellnum_list)
   ...
   for k = 1:length(cellfile_list_sorted)
    ...
    splitPath = split(folder_path, filesep);
    index1 = find(strcmp('BSL_Data3', splitPath));

    if ~isempty(index1)
    splitPath{index1} = 'BSL_Data2';
    else
    disp('The specified path segment ''Raw'' was not found.');
    end

    folder_path = strjoin(splitPath, filesep);
